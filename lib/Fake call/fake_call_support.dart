...
class _InstructionScreenState extends State<InstructionScreen> {
  ...
  bool _activateBtnStatus = true;

  static const values = <int>[0, 1, 2, 3];
  static const textValues = <String>['Now', '1 min', '3 min', '5 min'];

  // Make selectedValue nullable (int?)
  int? selectedValue = values.first;

  final selectedColor = Colors.green;
  final unselectedColor = Colors.white;

  late TextEditingController _fakeName; // Use late keyword to ensure initialization

  @override
  void initState() {
    super.initState();
    _fakeName = TextEditingController(); // Initialize the TextEditingController
  }
            ...
            child: Row(
              children: <Widget>[
                Expanded(
                  child: Radio<int>(
                    value: value,
                    groupValue: selectedValue ?? 0, // Use null-aware operator to avoid null
  ...
  void readyForCall() {
    if (_fakeName.text.isNotEmpty) {
      ...
      Future.delayed(Duration(minutes: selectedValue ?? 0), () {
        Navigator.pop(_scaffoldKey.currentContext!);
        Navigator.of(context).pushNamedAndRemoveUntil(FakeCallScreen.route, (Route<dynamic> route) => false);
      });

      Wakelock.disable();
    } else {
      showDialog<String>(context: _scaffoldKey.currentContext!,
